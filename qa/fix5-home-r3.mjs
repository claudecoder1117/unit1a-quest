// qa/fix5-home-r3.mjs — fixer r3 (lane home): the critic r2 scenarios re-run against the r3 fix. Harness copied from qa/fix5-home-r2.mjs. Output qa/screenshots/crit-home/r2/
...
const OUT = path.join(REPO, 'qa', 'screenshots', 'fix5-home', 'r3');
...
async function zeroPage() {
  const state = await readFile(path.join(REPO, 'qa', 'screenshots', 'crit-home', 'r2', 'zero.json'), 'utf8');
  const { ctx, page, errors } = await open({ route: '#/today', state });
...
async function placemix(dark = false) {
  const { ctx, page, errors } = await open({ route: '#/', dark });
  ...
  await shot(page, `placemix-summary-${dark ? 'dark' : 'light'}.png`, true); const rowsBox = await page.$('.ob-results'); if (rowsBox) { await rowsBox.scrollIntoViewIfNeeded(); await page.waitForTimeout(200); await shot(page, `placemix-rows-${dark ? 'dark' : 'light'}.png`); }
  const sk = await page.evaluate(() => JSON.parse(localStorage.getItem('u1a.save')).skills);
  say('skills', sk);
  say('READ', await READ(page));
  const hm = await HOME(page); say('HOME', hm);
  await writeFile(path.join(OUT, `placemix-${dark ? 'dark' : 'light'}.json`), await page.evaluate(() => localStorage.getItem('u1a.save')));
...
try {
  if (mode === 'placemix') await placemix(process.argv[3] === 'dark');
